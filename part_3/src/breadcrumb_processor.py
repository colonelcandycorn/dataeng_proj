...
class BreadCrumbProcessor:
    ...
    def process_individual_part3(breadcrumb: dict) -> pd.DataFrame | None:
        """
        Process breadcrumb and discard rows with null or negative values.
        """
        # read breadcrumb string into DataFrame
        bc_df = pd.DataFrame([breadcrumb])
        
        print(f"Row count: {bc_df.shape[0]}")

        # project columns
        bc_df = bc_df[['trip_id', 'route_number', 'vehicle_number', 'service_key', 'direction']]
        
        print(f"Row count after projection: {bc_df.shape[0]}")

        # filter out rows with null values
        bc_df = bc_df.dropna(how='any',axis=0)

        print(f"Row count after dropping nulls: {bc_df.shape[0]}")

        bc_df['trip_id'] = pd.to_numeric(bc_df['trip_id'], errors='coerce')
        bc_df['vehicle_number'] = pd.to_numeric(bc_df['vehicle_number'], errors='coerce')

        # filter out rows with negative values
        bc_df = bc_df[
            (bc_df['trip_id'] >= 0) &
            (bc_df['vehicle_number'] >= 0) 
            ]

        # rename columns
        bc_df = bc_df.rename(columns={'route_number': 'route_id', 'vehicle_number': 'vehicle_id'})

        # if the resulting DataFrame is empty, return None
        if bc_df.empty:
            return None
